...
outfolder='/scratch/x77/jm5970/ACCESS_trackeddy_3rd_cycle/post-processing/'
...
for ii in range(0,len(expts)):
   ...
   plt.pcolormesh(lon,lat,EKE_eddy[0,:,:],vmin=0,vmax=1000)
   plt.colorbar()
...
for tt in range(0,np.shape(eta)[0]):
    u_eddy[tt,:,:],v_eddy[tt,:,:]=geovelfield(eta[tt,:,:],lon,lat,mask,100)
    EKE_eddy[tt,:,:] = KE(u_eddy[tt,:,:],v_eddy[tt,:,:])
