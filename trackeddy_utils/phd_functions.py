...
def sat_seasonal_variation(year,monthsin,monthsend,areamap='',plot=True,hemisphere='',days=''):
    ...
    for ii in range(len(monthsin)):
        ...
        #print(ufilename)
        vfilename=Dataset('/home/156/jm5970/notebooks/trackeddy/data.input/ssha_v_satellite_'+str(year)+'_'+str(monthsin[ii])+'yrs.nc')
...
def seasonal_variation_EKEE(fileinit,fileend,areamap='',plot=True,hemisphere='',days=''):
    index=0
    if days=='':
        days=365
    eke_mean=np.zeros(days)
...
def seasonal_variation_EKE(fileinit,fileend,areamap='',plot=True,hemisphere='',days=''):
    index=0
    if days=='':
        days=365
    eke_mean=np.zeros(days)
...
def seasonal_variation_EKEB(fileinit,fileend,areamap='',plot=True,hemisphere='',days=''):
    index=0
    if days=='':
        days=365
    eke_mean=np.zeros(days)
    for ii in range(fileinit,fileend): 
        ...
        eke=np.zeros([np.shape(v_prime)[0],np.shape(v_prime)[1],np.shape(v_prime)[2]])
        for tt in range(0,np.shape(v_prime)[0]):
            eke_c=np.array(KE(u[tt,:,:]-u_prime[tt,:,:],v[tt,:,:]-v_prime[tt,:,:]))
            eke_mean[index]=np.nanmean(eke_c)
            index=index+1
    if plot==True:        
...
def sim_EKE_eddy_seasonal_variation(fileinit,fileend,areamap='',plot=True,hemisphere='',basemap=True,days=''):
    ...
    eke_mean=np.zeros(days)
    for ii in range(fileinit,fileend):
        ekefile=Dataset('/home/156/jm5970/notebooks/trackeddy/data.input/eke_eddy_'+str(ii)+'.nc')
...
def sim_EKE_back_seasonal_variation(fileinit,fileend,areamap='',plot=True,hemisphere='',basemap=True,days=''):
    ...
    eke_mean=np.zeros(days)
    for ii in range(fileinit,fileend):
        ekefile=Dataset('/home/156/jm5970/notebooks/trackeddy/data.input/eke_back_'+str(ii)+'.nc')
        lon=ekefile.variables['Longitude'][:]
        lat=ekefile.variables['Latitude'][:]
        #print(len(lon),len(lat))
        if areamap=='':
...
def sim_EKE_seasonal_variation(fileinit,fileend,areamap='',plot=True,hemisphere='',basemap=True,days=''):
    ...
    eke_mean=np.zeros(days)
    for ii in range(fileinit,fileend):
        ekefile=Dataset('/home/156/jm5970/notebooks/trackeddy/data.input/eke_'+str(ii)+'.nc')
        lon=ekefile.variables['Longitude'][:]
        lat=ekefile.variables['Latitude'][:]
        #print(len(lon),len(lat))
        if areamap=='':
