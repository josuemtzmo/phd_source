...
then
  ...
  then
    ...
    do
      ...
      do
	   ...
	   counter=$(( i*(file_divisions) + j))
           echo "${python_path} ${cdir}trackeddy_${dataorigin}_neg.py $((i+1)) $j $file_divisions $counter" > $cdir$rundir/config.$(printf %05d ${counter%})
    ...
    do
      ...
      do 
        echo "${python_path} ${cdir}trackeddy_${dataorigin}_neg.py $i ${months[$m]} ${months[$(($m+1))]} " > $cdir$rundir/config.$counter
